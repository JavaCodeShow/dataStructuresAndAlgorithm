...
public class KnapsackOptimal {
	public static void main(String[] args) {
		...
		knapsackOptimal(6, new int[] { 2, 7, 2 }, new int[] { 2, 7, 2 });
	}
	...
	public static void knapsackOptimal(int c, int[] weight, int[] value) {
		...
		/*
		 * for (int i = 1; i < values.length; i++) { values[i] = MIN; }
		 */
		...
		/*
		 * 输出顺序:逆序输出物品编号 注意：这里另外开辟数组G[i][v],标记上一个状态的位置 G[i][v] =
		 * 1:表示物品i放入背包了，上一状态为G[i - 1][v - w[i]] G[i][v] = 0:表示物品i没有放入背包，上一状态为G[i
		 * - 1][v]
		 */
		int i = n;
		int j = c;
